...
func myAtoi(str string) int {
	...
	for _, item := range []byte(str) {
		...
		t := int(item-'0') * negFlag
		if res*negFlag > 2147483647/10 || (res*negFlag == 2147483647/10 && t > 7) {
			return 2147483647
		}
		if res*negFlag < -2147483648/10 || (res*negFlag == -2147483648/10 && t < -8) {
			return -2147483648
		}
